...
// ir37-pink-tessen, ...) are ALSO registered here so the campaign can render them in-fight by
// `fighterId` and the Experience can resolve them for RENDERING unconditionally — but they are
// GATED in the charSelect PICK grid: a boss becomes a selectable tile only after its campaign node
// is beaten (see rosterGating.ts). The gate is UI-side only; this registry stays identity-agnostic.
