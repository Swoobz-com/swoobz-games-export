...
const OUT = 'shots-artotty-dark-0705'
...
await wait(800)
await page.screenshot({ path: `${OUT}/d1440-02-planning.png` })
const box = await canvasBox(page)
if (box) {
  const tile = box.w / 14
  for (let r = 12; r >= 4; r--) {
    await page.mouse.click(box.x + 5 * tile + tile / 2, box.y + r * tile + tile / 2)
    await wait(45)
  }
}
await wait(400)
...
// ---- WIN settle capture on DESKTOP 1440 — dense EARLY frames for cartouche/bloom ----
await page.setViewport({ width: 1440, height: 900, deviceScaleFactor: 1 })
let won = false
for (let attempt = 0; attempt < 12 && !won; attempt++) {
  ...
  const tile = b.w / 14
  for (let r = 1; r <= 8; r++) {
    await page.mouse.click(b.x + 5 * tile + tile / 2, b.y + r * tile + tile / 2)
    await wait(45)
  }
  await wait(200)
  ...
  const info = await page.evaluate(() => {
    ...
    return { won: /LINE CLAIMED/.test(t) && !/BUSTED|CRACKED DISC/.test(t), bust: /BUSTED|CRACKED DISC/.test(t), snip: t.slice(0, 140).replace(/\s+/g, ' ') }
  })
  if (info.won) {
    won = true
    await page.screenshot({ path: `${OUT}/WIN-settled.png` })
    for (const f of frames) fs.copyFileSync(f, f.replace('try' + attempt, 'WIN'))
    console.log('WON attempt', attempt, info.snip)
  } else {
