...
// DESKTOP 1440
{
  ...
  await wait(300)
  await page.screenshot({ path: `${OUT}/desktop-plot.png` })
  await clickText(page, /HADAL TRENCH/); await wait(350); await page.screenshot({ path: `${OUT}/desktop-hadal.png` })
  await clickText(page, /REEF SHELF/); await wait(350); await page.screenshot({ path: `${OUT}/desktop-reef.png` })
  ...
  await page.screenshot({ path: `${OUT}/desktop-settled-${res}.png` })
...
// MOBILE 412
{
  ...
  await page.goto(URL, { waitUntil: 'load', timeout: 60000 })
  await wait(900)
  await page.screenshot({ path: `${OUT}/mobile412-entry.png` })
  await clickText(page, /ENTER THE DIVE/)
  await wait(350)
  await traceLine(page, 10)
  await wait(300)
  await page.screenshot({ path: `${OUT}/mobile412-plot.png` })
  await page.close()
}

// iPHONE 393 x 852 (14 Pro) — fold check on planning
{
  const page = await browser.newPage()
  await page.setViewport({ width: 393, height: 852, deviceScaleFactor: 3, isMobile: true, hasTouch: true })
  await page.goto(URL, { waitUntil: 'load', timeout: 60000 })
  await wait(900)
  await page.screenshot({ path: `${OUT}/iphone393-entry.png` })
  await clickText(page, /ENTER THE DIVE/)
  await wait(350)
  await traceLine(page, 9)
  await wait(300)
  // above-the-fold only (no fullPage) — proves RUN THE LINE clears 852px
  await page.screenshot({ path: `${OUT}/iphone393-plot-fold.png` })
  const runVisible = await page.evaluate(() => {
    const el = [...document.querySelectorAll('button')].find(b => /RUN THE LINE/i.test(b.textContent || ''))
    if (!el) return 'no-cta'
    const r = el.getBoundingClientRect()
    return r.bottom <= 852 ? `visible(bottom=${Math.round(r.bottom)})` : `CLIPPED(bottom=${Math.round(r.bottom)})`
  })
  console.log('iphone393 RUN THE LINE:', runVisible)
  await page.close()
}
