...
const SHOTS = 'C:/Users/Erstr/AppData/Local/Temp/claude/C--Users-Erstr-OneDrive-Bureaublad-swoobz-games-export/eed49dad-7609-4969-843a-cb4e422d24da/scratchpad/shots'
...
async function measureReceipt(page) {
  return page.evaluate(() => {
    ...
    const headline = all.find((d) => /WRECK RECKONING/.test(d.textContent) && d.textContent.length < 130)
    ...
    return {
      ...
      seal: R(seal), payout: R(payoutValue), headline: R(headline), copy1: R(copyBtns[0]), copy2: R(copyBtns[1]),
      copy1Text: copyBtns[0]?copyBtns[0].textContent.trim():null, copy2Text: copyBtns[1]?copyBtns[1].textContent.trim():null,
...
const browser = await puppeteer.launch({ executablePath: EXE, headless: false, defaultViewport: null, args: ['--autoplay-policy=no-user-gesture-required','--no-sandbox','--window-size=1500,1000'] })
...
for (const vp of VIEWPORTS) {
  ...
  for (let attempt = 0; attempt < 24 && !done; attempt++) {
    ...
    if (settledText && /SECURED THE HAUL/i.test(settledText)) {
      ...
      const sealVsCopy2 = rectInter(m.seal, m.copy2)
      const payoutVsCopy1 = rectInter(m.payout, m.copy1)
      const sealVsHeadline = rectInter(m.seal, m.headline)
      const sealBot2copy1 = (m.seal&&m.copy1)?+(m.copy1.top-m.seal.bottom).toFixed(1):null
      await page.screenshot({ path: `${SHOTS}/receipt-${vp.name}.png` })
      { const bx=[m.payout,m.seal,m.headline,m.copy1,m.copy2].filter(Boolean); if(bx.length){ const L=Math.max(0,Math.min(...bx.map(b=>b.left))-18), T=Math.max(0,Math.min(...bx.map(b=>b.top))-16), Rr=Math.max(...bx.map(b=>b.right))+18, B=Math.max(...bx.map(b=>b.bottom))+16; await page.screenshot({ path: `${SHOTS}/crop-${vp.name}.png`, clip:{x:L,y:T,width:Rr-L,height:B-T} }) } }
      report.fix1.push({ viewport: vp.name, ...m, sealVsPayout, sealVsCopy1, sealVsCopy2, copy1VsCopy2, payoutVsCopy1, sealVsHeadline, sealBot2copy1 })
      console.log(`[FIX1][${vp.name}] payout="${m.payoutText}" copy1="${m.copy1Text}" copy2="${m.copy2Text}"`); console.log(`   seal_PAYOUT=${sealVsPayout.area} seal_copy1=${sealVsCopy1.area} seal_copy2=${sealVsCopy2.area} copy1_copy2=${copy1VsCopy2.area} payout_copy1=${payoutVsCopy1.area} seal_headline=${sealVsHeadline.area} sealBot->copy1Top=${sealBot2copy1}`)
      done = true
    }
    await page.close()
  }
