...
const tiles = await page.evaluate(() => {
  ...
  const wrap = document.querySelector('.assayBoardScroll')
  ...
  for (let r = 0; r < GRID; r++) {
    for (let col = 0; col < GRID; col++) {
      ...
      if (x >= wr.left + 3 && x <= wr.right - 3 && y >= wr.top + 3 && y <= wr.bottom - 3) {
        visible.push({ r, col, x, y })
      }
    }
  }
  return { visible, wrapRect: { top: wr.top, left: wr.left, right: wr.right, bottom: wr.bottom } }
})
console.log('wrapRect', tiles.wrapRect, 'candidates', tiles.visible.length)
const picks = tiles.visible.slice(0, 8)
