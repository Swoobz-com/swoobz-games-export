...
// scroll-aware, works on the mobile pan-window board
...
async function testCopyFires(page, btnIndex) {
  // measure rect + attempt a real touchscreen tap on the CopyGlyph, confirm
  // (a) label text flips 'copy' -> 'copied', (b) clipboard actually received the text
  const before = await page.evaluate((idx) => {
    ...
    return { rect: { left: +r.left.toFixed(1), top: +r.top.toFixed(1), w: +r.width.toFixed(1), h: +r.height.toFixed(1) }, label: b.textContent.trim(), expect: (b.getAttribute('aria-label') || '').slice('Copy '.length) }
  }, btnIndex)
  ...
  return { found: true, rect: before.rect, labelBefore: before.label, labelAfter: after, expectedText: before.expect, clipboardText: clip, clipboardMatches: clip === before.expect }
}
...
for (const vp of VIEWPORTS) {
  ...
  // grant clipboard perms up front for this origin
  const ctx = browser.defaultBrowserContext()
  await ctx.overridePermissions(URL, ['clipboard-read', 'clipboard-write'])

  // ===== A: settled receipt cluster (overlap + copy-button fire) =====
  {
    let done = false
    for (let attempt = 0; attempt < 30 && !done; attempt++) {
      ...
      if (settledText && /SECURED THE HAUL/i.test(settledText)) {
        ...
        const overflowY = m.docScrollH > m.innerH + 1 // informational only, page can legit scroll
        ...
        // re-confirm restart works: tap DIVE AGAIN (narrow layout inline CTA)
        const beforeRestartText = await page.evaluate(() => document.body.innerText)
        const restartTap = await tapButtonByText(page, /DIVE AGAIN/i)
        await wait(300)
        const afterRestartText = await page.evaluate(() => document.body.innerText)
        const restartWorked = restartTap && !/SECURED THE HAUL|RUGGED BY THE DEEP/i.test(afterRestartText) && beforeRestartText !== afterRestartText

        report[vp.name].receipt = {
          m, sealVsPayout, sealVsCopy1, payoutVsCopy1, copy1VsCopy2,
          overflowX, overflowY,
          copy1Fire, copy2Fire, restartTap, restartWorked,
        }
        console.log(`[${vp.name}] payout="${m.payoutText}" seal∩PAYOUT=${sealVsPayout.area} seal∩copy1=${sealVsCopy1.area} payout∩copy1=${payoutVsCopy1.area} copy1∩copy2=${copy1VsCopy2.area} overflowX=${overflowX}`)
        console.log(`[${vp.name}] copy1: rect=${JSON.stringify(copy1Fire.rect)} labelBefore=${copy1Fire.labelBefore} labelAfter=${copy1Fire.labelAfter} clipMatches=${copy1Fire.clipboardMatches}`)
        console.log(`[${vp.name}] copy2: rect=${JSON.stringify(copy2Fire.rect)} labelBefore=${copy2Fire.labelBefore} labelAfter=${copy2Fire.labelAfter} clipMatches=${copy2Fire.clipboardMatches}`)
        console.log(`[${vp.name}] restartWorked=${restartWorked}`)
        done = true
        await page.close()
      } else {
        await page.close()
    ...
    if (!done) { console.log(`[${vp.name}] WARNING: no WIN in 30 attempts`); report[vp.name].receipt = { error: 'no-win' } }
  }

  // ===== B: active-phase delta chip vs depth label =====
  {
    const page = await browser.newPage()
    await page.setViewport({ width: vp.width, height: vp.height, deviceScaleFactor: 1, isMobile: true, hasTouch: true })
    await page.goto(URL, { waitUntil: 'load' })
    ...
    report[vp.name].activeChip = { sawChip: res.sawChip, worstOverlap: res.worst, labelRect: res.labelRect }
    console.log(`[${vp.name}] ACTIVE sawChip=${res.sawChip} worst delta∩zone area=${res.worst.area || 0}`)
    await page.close()
  }
}
