...
(async () => {
  ...
  const count = await page.evaluate(() => [...document.querySelectorAll('button')].filter(b=>b.textContent.toLowerCase().includes('bet again')).length);
  console.log('mobile BET AGAIN button count:', count);
  await browser.close();
})();
