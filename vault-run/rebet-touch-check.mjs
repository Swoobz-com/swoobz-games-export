...
const report = await page.evaluate(() => {
  function info(el) {
    ...
    return { width: Math.round(r.width), height: Math.round(r.height), touchAction: cs.touchAction };
  }
  const decBtn = document.querySelector('button[aria-label="Decrease next bet"]');
  const incBtn = document.querySelector('button[aria-label="Increase next bet"]');
  ...
  return {
    decreaseStepper: info(decBtn),
    increaseStepper: info(incBtn),
    nearBoardBetAgain: info(nearBtn),
    sidebarBetAgain: info(sidebarBtn),
  };
});
console.log('TOUCH-TARGET REPORT', JSON.stringify(report, null, 1));
