...
async function playOneRound(page, firstRound) {
  if (firstRound) {
    ...
    await clickText(page, 'bet again'); await wait(400);
  }
...
function r(el){ if(!el) return null; const rc = el.getBoundingClientRect(); return {top:rc.top,left:rc.left,right:rc.right,bottom:rc.bottom,width:rc.width,height:rc.height}; }
(async () => {
  ...
  const data = await page.evaluate(() => {
    ...
    const stack = document.querySelector('[data-testid="vault-gutter-right"]');
    ...
    return { shell: rr(shell), gutterB: rr(gutterB), gutterC: rr(gutterC), stack: rr(stack) };
  });
  console.log(JSON.stringify(data, null, 2));
  await page.screenshot({ path: 'shots/probe-cardb-settled.png', fullPage: false });
  await browser.close();
})();
