...
(async () => {
  ...
  const receiptChip = await page.evaluate(() => !!document.querySelector('.vault-receipt-toggle'));
  console.log('receiptChip', receiptChip);
  const buttons = await page.evaluate(() => [...document.querySelectorAll('button')].map(b => ({t: b.textContent.trim(), vis: b.offsetParent !== null})));
  console.log('buttons', JSON.stringify(buttons));
  await page.screenshot({ path: 'shots-vaultfix3-0704/mobdiag3-settled.png', fullPage: true });
  const clicked = await clickText(page, 'view receipt');
  console.log('clickedViewReceipt', clicked);
  await wait(500);
  const dlInfo = await page.evaluate(() => {
    const dls = [...document.querySelectorAll('dl')];
    return dls.map(d => ({html: d.outerHTML.slice(0,120)}));
  });
  console.log('dls', JSON.stringify(dlInfo));
  await page.screenshot({ path: 'shots-vaultfix3-0704/mobdiag3-afterclick.png', fullPage: true });
  await browser.close();
})();
