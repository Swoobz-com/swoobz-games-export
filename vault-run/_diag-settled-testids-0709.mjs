...
try {
  ...
  console.log('settled after', guard, 'taps:', settled)
  await wait(1500)
  const allTestIds = await p.evaluate(() => [...document.querySelectorAll('[data-testid]')].map(e => e.getAttribute('data-testid')))
  console.log('ALL TESTIDS:', JSON.stringify(allTestIds, null, 2))
  const betAgainByText = await p.evaluate(() => {
    const el = [...document.querySelectorAll('button')].find(b => /bet again/i.test(b.textContent||''))
    if (!el) return null
    return { text: el.textContent.trim(), testid: el.getAttribute('data-testid'), outerHTML: el.outerHTML.slice(0,300) }
  })
  console.log('BET AGAIN by text:', JSON.stringify(betAgainByText, null, 2))
  await p.screenshot({ path: '_diag-settled-testids-0709.png' })
} finally {
