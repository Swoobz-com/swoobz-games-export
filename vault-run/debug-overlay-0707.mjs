...
await wait(1000)
for (const idx of [0, 29, 24]) {
  const cell = await cellCenter(p, idx, 7, true)
  await p.evaluate(({cx,cy,idx}) => {
    const d = document.createElement('div')
    d.style.cssText = `position:fixed;left:${cx-4}px;top:${cy-4}px;width:8px;height:8px;background:red;z-index:99999;border-radius:50%`
    d.setAttribute('data-marker', String(idx))
    document.body.appendChild(d)
  }, {cx: cell.cx, cy: cell.cy, idx})
}
await p.screenshot({ path: 'shots-consolidated-0707/debug-overlay-markers.png' })
await b.close()
