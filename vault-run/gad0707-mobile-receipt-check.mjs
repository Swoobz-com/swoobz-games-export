...
const clicked = await p.evaluate(() => {
  ...
  const el = els.find((e) => /view receipt/i.test(e.textContent || ''))
  if (el) { el.click(); return true }
  return false
})
await wait(500)
// scroll to bottom to reveal expanded receipt
await p.evaluate(() => window.scrollTo(0, document.body.scrollHeight))
await wait(300)
await p.screenshot({ path: 'gad0707-mobile-receipt-expanded.png', fullPage: true })
const text = await p.evaluate(() => document.body.innerText)
console.log('clickedReceipt', clicked)
console.log('contains mixer:', /mixer/i.test(text))
console.log('contains server seed:', /server seed/i.test(text))
await b.close()
console.log('done')
