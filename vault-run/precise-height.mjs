...
const report = await page.evaluate(() => {
  function precise(el) {
    ...
    return { width: r.width, height: r.height, lineHeight: cs.lineHeight, fontSize: cs.fontSize, padding: cs.padding };
  }
  ...
  return { nearBoardBetAgain: precise(nearBtn), decreaseStepper: precise(decBtn) };
});
console.log('PRECISE', JSON.stringify(report, null, 1));
await browser.close();
