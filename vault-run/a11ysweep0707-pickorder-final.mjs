...
async function run(world, g){
  ...
  let out=null;
  for(let attempt=0; attempt<10 && !out; attempt++){
    ...
    await wait(700);
    // click TWO different tiles (increase odds of at least 1 safe reveal before any mine)
    for(let k=0;k<2;k++){
      const settled=await page.evaluate(()=>!!document.querySelector('[data-testid="vault-settledpanel"]'));
      if(settled) break;
      const c=await page.evaluate(({idx,g})=>{const cv=document.querySelector('canvas');const r=cv.getBoundingClientRect();const W=r.width,H=r.height;const tR=H*0.15,bR=H*0.18,sF=0.08;const sW=W*(1-sF*2),sH=(H-tR-bR)*0.96;const av=Math.min(sW,sH);const gap=Math.max(6,av*0.026);const tile=(av-gap*(g-1))/g;const full=tile*g+gap*(g-1);const x0=(W-full)/2;const by=tR+(H-tR-bR)/2;const y0=by-full/2;const col=idx%g,row=Math.floor(idx/g);return{cx:r.left+x0+col*(tile+gap)+tile/2,cy:r.top+y0+row*(tile+gap)+tile/2};},{idx:attempt*2+k,g});
    ...
    if(uniq.length){
      ...
      for(const e of uniq){
        ...
        fs.writeFileSync(`shots-a11ysweep-2026-07-07/pickorder-${world}-${e.t}-FINAL-tight.png`,buf);
        const png=PNG.sync.read(Buffer.from(buf)); const{minC,maxC}=extremes(png);
        samples.push({text:e.t, ratio:+ratio(maxC,minC).toFixed(2), fg:maxC, bg:minC});
        const wide={x:Math.max(0,pageX-20),y:Math.max(0,pageY-20),width:40,height:40};
        const bufw=await page.screenshot({clip:wide});
        fs.writeFileSync(`shots-a11ysweep-2026-07-07/pickorder-${world}-${e.t}-FINAL-wide.png`,bufw);
      }
      out={attempt,samples};
    }
    await page.close();
...
const world=process.argv[2], g=parseInt(process.argv[3]);
