...
const m = await page.evaluate(() => {
  ...
  const kids = [...pageDiv.children];
  const first = kids[0].getBoundingClientRect();
  const last = kids[kids.length-1].getBoundingClientRect();
  const cabRect = cabinet.getBoundingClientRect();
  const vw = window.innerWidth, vh = window.innerHeight;
  return { vw, vh, contentTop:first.top, contentBottom:last.bottom, contentH: last.bottom-first.top, cabW:cabRect.width, cabH:cabRect.height, nKids: kids.length };
});
const deadTotal = m.vh - m.contentH;
...
console.log(TAG, 'cabinet width fill pct of vw:', (m.cabW/m.vw*100).toFixed(1)+'%');
await browser.close();
