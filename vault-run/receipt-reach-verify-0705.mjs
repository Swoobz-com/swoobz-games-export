...
(async () => {
  ...
  const data = await page.evaluate(() => {
    const body = document.getElementById('vault-gutter-settled-receipt');
    const toggleBtn = [...document.querySelectorAll('button')].find(b => b.textContent.toLowerCase().includes('receipt'));
    ...
    return {
      ...
      toggleClientRects: toggleBtn?.getClientRects().length,
    };
  });
  await page.screenshot({ path: 'shots-gad-defect-0705-afterfix/receipt-expanded-1440.png' });
