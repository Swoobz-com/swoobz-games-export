...
await wait(1500); // allow verifyState to settle from 'verifying' -> 'matched'
const toggled = await clickText('view receipt');
...
const info = await page.evaluate(() => {
  ...
  return { found: true, text: el.textContent.slice(0,200), position: cs.position, visible: el.offsetParent !== null, rect: { top: Math.round(r.top), bottom: Math.round(r.bottom) } };
});
console.log('RECEIPT AFTER TOGGLE', JSON.stringify(info, null, 1));
const fixedNow = await page.evaluate(() => [...document.querySelectorAll('body *')].filter(e => getComputedStyle(e).position === 'fixed' && e.offsetParent !== null).length);
console.log('fixed-position overlay count with receipt expanded:', fixedNow);
await browser.close();
