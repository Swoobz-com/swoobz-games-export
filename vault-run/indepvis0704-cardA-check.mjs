...
async function check(browser, w, h, tag) {
  ...
  await page.screenshot({ path: `${OUT}/cardA-${tag}-lobby.png`, fullPage: true });
  await page.close();
  return {
    canvasRect, panelLeftViewport, panelRightViewport,
    cardALeft, cardARight,
...
(async () => {
  ...
  const R = {};
  for (const w of [960, 1024, 1100, 1150, 1440, 1920]) {
    R[`w${w}`] = await check(browser, w, w >= 1400 ? (w === 1920 ? 1080 : 900) : 800, `w${w}`);
  }
  await browser.close();
  fs.writeFileSync(`${OUT}/cardA-results.json`, JSON.stringify(R, null, 2));
