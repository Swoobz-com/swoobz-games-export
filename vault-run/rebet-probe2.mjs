...
await wait(700);
console.log('click send it', await clickText('send it'));
await wait(900);
...
for (let k = 0; k < 25 && !settledNow; k++) {
  ...
  if (settledNow) console.log('settled at tile', k);
}
console.log('settledNow', settledNow);
await wait(500);
...
await page.screenshot({ path: 'shots/rebet-probe2-after.png' });
await browser.close();
