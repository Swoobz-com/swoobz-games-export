...
clips and iOS Safari will not play video from a server that answers a Range with a full 200), an
extension-gated SPA fallback, and a resolve+separator traversal guard. **`ws` moved to `dependencies`** —
